...
contract SignatureContract is Ownable {
    // A FantomSign contract represents a esignature processto be completed.
    
    string private title; // Title of the fantomsign contract.
    string private resourceUrl; // Link to the fantomsign documents to be signed.
    ...
    constructor(string memory _title,  address _signerAddress) {
        console.log("Deploying a FantomSign contract with title:", _title);
        title = _title;
    ...
    function markCompleted(string memory _signatureUrl) public {
        ...
        require(address(msg.sender) == signerAddress, "Only the designated signer can complete the contract");
